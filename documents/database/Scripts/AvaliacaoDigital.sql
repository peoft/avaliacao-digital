...
CREATE TABLE IF NOT EXISTS `avaliacaoDigital`.`formulario` (
  ...
  `avaliacaoCodigo` VARCHAR(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `avaliacaoDigital`.`avaliacaoTopico` (
  ...
  PRIMARY KEY (`codigo`, `avaliacaoCodigo`, `topicoCodigo`),
  INDEX `avalidacaoCodigo_idx` (`avaliacaoCodigo` ASC),
  INDEX `codigo_idx` (`topicoCodigo` ASC),
  CONSTRAINT `fkAvalidacaoCodigo`
    FOREIGN KEY (`avaliacaoCodigo`)
    REFERENCES `avaliacaoDigital`.`avaliacao` (`codigo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `avaliacaoDigital`.`topicoQuestao` (
  ...
  PRIMARY KEY (`questaoCodigo`),
  INDEX `fkQuestaoCodigo_idx` (`questaoCodigo` ASC),
  UNIQUE INDEX `questaoCodigo_UNIQUE` (`questaoCodigo` ASC),
...
-- -----------------------------------------------------
-- Table `avaliacaoDigital`.`administrador`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `avaliacaoDigital`.`administrador` (
  `usuarioCodigo` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `tempoUso` TIME(5) NOT NULL,
  `ultimaOperacao` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`usuarioCodigo`),
  CONSTRAINT `fkUsuarioCodigo`
    FOREIGN KEY (`usuarioCodigo`)
    REFERENCES `avaliacaoDigital`.`usuario` (`codigo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
